<script lang="ts">
	import type { Experience, ExperienceResponsibility, ExperienceProject } from "@prisma/client";
  ...
  const getExperienceDateRange = (fromDate: Date, toDate: Date | null): string => {
          ...
          {#if experience.projects.length !== 0}
            <ul class="pt-4 list-disc text-neutral-500">
              ...
              {#each experience.projects as project}
                <li class="ml-4">
                  {#if project.url}
                    <a href={project.url} class="hover:text-black underline font-bold">{project.name}</a>: {project.description}
                  {:else}
