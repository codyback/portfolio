...
<aside transition:fly={{x: 640, opacity: 1}} class="absolute w-full h-full z-10 bg-gray-200 border-l-2 shadow-lg" class:open>
    ...
    <a href="/blog" class="px-2 py-3 block rounded-md hover:bg-slate-200 leading-10" on:click={() => open = !open}>BLOG</a>
    <a href="/resources" class="px-2 py-3 block rounded-md hover:bg-slate-200 leading-10" on:click={() => open = !open}>RESOURCES</a>
	</nav>
</aside>
{/if}
